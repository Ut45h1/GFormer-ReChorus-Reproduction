...
class LightGCNBase(object):
    ...
    def build_adjmat(user_count, item_count, train_mat, selfloop_flag=False):
        # =======================================================
        # 回归标准版图构建逻辑 (适合 LastFM/ML-1M，不适合 Yelp)
        # 使用 dok_matrix，逻辑简单清晰
        # =======================================================
...
class LGCNEncoder(nn.Module):
    ...
    def forward(self, users, items):
        # ==========================================================
        # 核心保留：运行时强制初始化检测 (Hack Fix for Zero Init Issue)
        # 这是为了解决你 WSL/Windows 环境下参数被清零的问题，必须保留！
        # ==========================================================
        if torch.abs(self.embedding_dict['user_emb']).sum() < 1e-5:
            # 只有在第一次或参数意外丢失时才会触发
            # print("\n[WARN] LightGCN Embedding 被清零！执行强制重置...") 
            # 注释掉 print 防止刷屏，但功能保留
            with torch.no_grad():
                nn.init.xavier_normal_(self.embedding_dict['user_emb'])
                nn.init.xavier_normal_(self.embedding_dict['item_emb'])
        # ==========================================================
